...
$published	=	DB::table('eoi_request')
				->where('userid',session('userId'))
				->where('eoistatus','>=',3)
				->where('isClosed','=',0)
				->where('iscancelled',0)
		...
		<div class="col-sm-12">
			<div class="col-sm-12">
				...
				<div class="col-sm-3 animated flipInX delay-02">
					<a class="dashboard-card-link" href="{{route('dept.eoilist',1)}}">
						<div class="dashboard-horizontal-card role-dashboard-card bg-orange">
							...
							<div class="role-card-body">
								...
								<p class="dashboard-card-title">Draft EoI</p>
							</div>
						</div>
					</a>
				</div>
				...
				<div class="col-sm-3 animated flipInX delay-03">
					<a class="dashboard-card-link" href="{{route('dept.eoilist',2)}}">
						<div class="dashboard-horizontal-card role-dashboard-card bg-blue">
							<span class="dashboard-icon"><i class="fa fa-folder-open-o"></i></span>
							<div class="role-card-body">
								<div class="dashboard-card-count">{{$approved}}</div>
								<p class="dashboard-card-title">Approved by Department</p>
							</div>
						</div>
					</a>
				</div>
				...
				<div class="col-sm-3 animated flipInX delay-04">
					<a class="dashboard-card-link" href="{{route('dept.eoilist',3)}}">
				...
				<div class="col-sm-3 animated flipInX delay-05 mt-20">
					<a class="dashboard-card-link" href="{{route('department.orderslist')}}">
						...
						<ul class="notification-list">
							@foreach($responses as $response)
								<li style="list-style:none;">
									<a href="{{route('show.prebidanswer', Crypt::encrypt($response->broadcastid))}}"
										class="form-label font-12">
										<i class="fa fa-hand-o-right"></i> {{$response->eoinumber}} <i style="padding:0px 10px;">Date :
...
<script>
	setTimeout(function () { loadData('{{ route('departmentdashboard.html') }}'); }, 1000);

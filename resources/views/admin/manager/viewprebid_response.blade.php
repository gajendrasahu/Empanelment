...
<table class="mytable" border="1" style="text-transform: none!important;">
	...
	@if($data)
		<tr>
			<td class="padding-5" style="position:relative;">
			Posted On : {{date('d\-m\-Y, h:i A',strtotime($data->broadcastedon))}}</b><br><br>
			{!!$data->broadcastmessage!!}
			@if($data->attachment!='')
			<br><br>
			<a href="{{route('view.uploadedfile',Crypt::encrypt($data->attachment))}}" target="_blank" style="position:absolute; top:15px; right:15px;">
				<span class="label label-white middle animated flipInX f-s-10 form-label">
					<i class="ace-icon fa fa-file-pdf-o bigger-120 icon-animated-bell"></i> View File
				</span>

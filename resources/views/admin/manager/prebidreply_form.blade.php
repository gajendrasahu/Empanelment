...
<button type="button"
    ...
    data-url="{{route('updateprebid.pmresponse',Crypt::encrypt($eoi->requestid))}}"
...
@endif
	...
	<form name="submitResponse" id="submitResponse" action="{{route('submitprebid.pmresponse',Crypt::encrypt($eoi->requestid))}}" method="post" enctype="multipart/form-data">
	@csrf
	@if(!$replied)
	<table class="mytable pd-8" border="1">

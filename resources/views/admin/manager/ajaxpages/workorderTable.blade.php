	...
	<td class="padding-2 eoi-card-cell" style="@if($serial%2!=0) background-color:white!important; @endif">
		<div class="eoi-card">
			...
			<table style="border:none!important; width:100%!important;">
				...
				@endif
				@if($item->notesheet!='')
				<a href="{{ route('view.uploadedfile',Crypt::encrypt($item->notesheet)) }}" class="eoi-btn action-a" target="_blank">
						<i class="fa fa-file-pdf-o"></i> Note Sheet
				</a>
				@endif
				@if($item->markingsheet!='')
				<a href="{{ route('view.uploadedfile',Crypt::encrypt($item->markingsheet)) }}" class="eoi-btn action-a" target="_blank">
						<i class="fa fa-file-pdf-o"></i> Marking Sheet
				</a>
				@endif
				@if($item->momfile!='')
				<a href="{{ route('view.uploadedfile',Crypt::encrypt($item->momfile)) }}" class="eoi-btn action-a" target="_blank">
						<i class="fa fa-file-pdf-o"></i> MoM
				</a>
				@endif
				@if($item->isActiveOrder==1 && \Carbon\Carbon::parse($item->workorderduedate)->gte(\Carbon\Carbon::today()))
				<a href="{{ route('view.pmdeployment',Crypt::encrypt($item->orderid)) }}" class="eoi-btn action-a">
...
const paginationLinks = document.querySelectorAll('.pagination a');
        paginationLinks.forEach(link => {
            link.onclick = function(event) {
                ...
                const url = '{{ route('pmorderlist.html') }}'; // URL generated by Laravel route

...
<table class="mytable pd-8" border="1">
	...
	<tr>
		...
		<td class="center">
			<input type="text" name="update_{{$att->attendance_id}}" id="" value="{{$att->remarks}}" class="form-label font-normal" style="width:100%;" placeholder="Remark (if any)">
		</td>
	</tr>
	@endforeach
		...
		<td style="width:40%; text-align:left;" class="v-top">
			<table class="mytable pd-5 font-14">
				...
				<tr class="form-label">
					...
					<td class="center">
						<input type="text" name="holidays" id="holidays" value="{{$summary->total_holiday}}" placeholder="0" class="width-50 center" onchange="CalculateSalary()" autocomplete="off" readonly>
					</td>
				</tr>
			</table>
			<table class="mytable pd-5 font-14 mt-20">
				...
				<tr>
					...
					<td style="width:150px;"><input type="text" class="numbers" value="{{number_format(round($summary->net_salary),'2','.','')}}" name="net_salary" id="net_salary" readonly style="width:100%;"></td>
				</tr>
				<tr>
					<td class="form-label">Approved Salary</td>
					<td><input type="text" class="numbers" value="@if($summary->approved_salary){{number_format(round($summary->approved_salary),'2','.','')}}@else {{number_format(round($summary->net_salary),'2','.','')}} @endif" name="calculated_salary" id="calculated_salary" readonly style="width:100%;"></td>
				</tr>
			</table>
		</td>
...
$(".modal-footer").append(
  '<button type="button" class="btn btn-info goAheadBtn gridbtn width-100" onclick="viewMprManager(\'{{ route('view.mprmanager') }}\', \'{{ Crypt::encrypt($mpr->mpr_id) }}\')">' +
...
$(".modal-footer").append(
  '<button type="button" class="btn btn-info goAheadBtn gridbtn width-100 aprBtn" onclick="approveResourceMpr(\'{{ route('approve.pmresourcempr') }}\', \'{{ Crypt::encrypt($summary->mpr_id) }}\', \'{{ Crypt::encrypt($summary->deployment_id) }}\')">' +
		...
		if(result)
		{
			...
			$.ajax({
				...
				success: function(response) {
					...
					{
						$('#mprContent').html('<div style="width:100%; text-align:center;"><h2><i class="fa fa-thumbs-up"></i><br><br>Attendance & MPR detail approved successfully!</h2></div>');
						
						setTimeout(function(){
							$.get('{{route('view.mprmanager')}}',
							{
								mprid:mprid,
							},
			...
			$.ajax({
				...
				success: function(response) {
					if(response.status==200)
					{
						...
						setTimeout(function(){
							$.get('{{route('view.attendancemprmanager')}}',
							{
								mprid:mprid,

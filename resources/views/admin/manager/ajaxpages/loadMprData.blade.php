...
<table class="mytable pd-8" border="1">
	...
	<tr class="myheadbg form-label">
		...
		<td class="font-bold">@if($order->categoryid==1) Role & Experience @else Position & Sector @endif</td>
	...
	<tr>
		...
		<td nowrap class="v-top">
			@if($record->sectorname)
				{{ucwords(strtolower($record->sectorname))}}<br>{{ucwords(strtolower($record->consultantposition))}}
			@else
				{{$record->role}}<br>{{$record->experience}}, Level - {{$record->experiencelevel}}
			@endif
		</td>
		...
		<td class="center v-top">{{$record->total_present+$record->total_absent+$record->total_leave+$record->total_holiday}}</td>
		...
		<span class="btn btn-info gridbtn" onclick="viewAttendanceMprManager('{{route('view.attendancemprmanager')}}','{{Crypt::encrypt($record->mpr_id)}}','{{Crypt::encrypt($record->deployment_id)}}')">
...
function viewAttendanceMprManager(r1,mprid,deploymentid)
{
	$.get(""+r1,
	{
		mprid:mprid,

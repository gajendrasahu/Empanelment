...
@csrf
	<div class="row">
		<div class="col-sm-3">
			<input type="hidden" name="form_token" id="form_token" readonly value="{{$token}}">
			...
			<input name="position_role" id="position_role" class="form-control" value="{{$record->role}}" required>
		</div>
		<div class="col-sm-3">
			<span class="form-label">Experience (As per empanelment)<label id="req">*</label></span>
			<select name="resource_experience" id="resource_experience" class="form-control">
				@foreach($experiences as $experience)
				<option value="{{$experience->workexperience}}" @if($experience->workexperience==$record->experience) selected @endif >{{$experience->workexperience}}</option>
				@endforeach
		...
		<div class="col-sm-2">
			<span class="form-label">Experience Level <label id="req">*</label></span>
			<select name="experience_level" id="experience_level" class="form-control">
				@foreach($levels as $level)
				<option value="{{$level->experiencelevel}}" @if($level->experiencelevel==$record->experiencelevel) selected @endif >Level {{$level->experiencelevel}}</option>
				@endforeach
...
$('.submitUpdateBtn').on('click', function(e) {
	...
	bootbox.confirm('Once you click OK, the data will be updated and cannot be reverted. Do you want to proceed?',function(result){
		if(result)
		{
			...
			$.ajax({
				url: '{{ route("log.pmupdateresource") }}',
				method: 'POST',
				data: formData,
				processData: false,
				contentType: false,
				success: function(response) {
					...
					if (response.status === 200)
					{
						...
						setTimeout(function(){ GetUpdateForm('{{route("load-pmupdateform")}}'); $('#editContent').html(""); $('#editModal').modal('hide'); },3000);
					}

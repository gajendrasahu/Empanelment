...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="form-title">EoI Draft</div>
        </div>
            ...
            <table class="requirements-table">
				...
				<tfoot>
					<tr>
						...
						<td id="total-cell" class="f-right info-label"><i class="fa fa-inr"></i> {{$total}}</td>
					</tr>
					<tr>
						<td class="f-right info-label" colspan="6">CHiPS Admin Charge ({{$admincharge}}%)</td>
						<td id="admincost-cell" class="f-right info-label">{{$admincost}}</td>
					</tr>
					<tr class="total-budget">
						<td class="f-right info-label" colspan="6">Grand Total</td>
						<td id="grand-cell" class="f-right info-label">{{$total+$admincost}}</td>
					</tr>
				</tfoot>
            </table>
        </div>
...
<script>
    window.addEventListener('DOMContentLoaded', function () {
        ...
        let processed 	= 	formatIndianNumber(total);
        document.getElementById('total-cell').innerText = processed;

        let admincost 		= 	{{ json_encode($admincost) }};
        let processed1 		= 	formatIndianNumber(admincost);
        document.getElementById('admincost-cell').innerText = processed1;

        let grand 		= 	{{ json_encode($total+$admincost) }};
        let processed2 		= 	formatIndianNumber(grand);
        document.getElementById('grand-cell').innerText = processed2;
    });
</script>	

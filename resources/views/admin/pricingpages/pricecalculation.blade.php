...
<tr>
	<th colspan="8" class="text-left">Pricing Details - {{$tier->tiername}} @if($startDate)(Effective from {{date('d\-m\-Y',strtotime($startDate))}})@endif</th>
</tr>
<tr>
	@if($categoryid==2)
...
<tr>
	...
	<td class="v-top text-left" rowspan="{{ $rowCount }}">{{ $resource['resource'] }}</td>
	@endif
	@if($categoryid==1)

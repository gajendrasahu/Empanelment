...
@csrf
	<div class="row">
		...
		<div class="col-sm-2">
			...
			<select name="positionid" id="positionid" class="form-control" onchange="setResourceExperience()">
		...
		<div class="col-sm-12">
			...
			<textarea name="qualification" id="qualification" class="form-control tinymce">{!!$record->qualification!!}</textarea>
		</div>

		<div class="col-sm-12">&nbsp;</div>
		<div class="col-sm-12">
			<span class="form-label">Remark <label id="req">*</label></span>
			<textarea name="remark" id="remark" class="form-control tinymce">{!!$record->remark!!}</textarea>
		</div>
		<div class="col-sm-12">&nbsp;</div>
...
{
    var experience = $('#positionid option:selected').data('value');
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (response.status === 200)
					{
						...
						setTimeout(function(){ GetForm('{{route("load-form")}}'); $('#editContent').html(""); $('#editModal').modal('hide'); },3000);
					}

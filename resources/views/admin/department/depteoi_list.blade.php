...
<div class="tabbable animated bounceInUp" style="padding:0px 0px!important;">
		@if(in_array(2,Session::get('actions')) || in_array(3,Session::get('actions')) || in_array(12,Session::get('actions')))
			<div class="eoi-section-header" onclick="loadData(1,'{{ route('depteoirequest.html') }}')">
				<div style="font-size:20px; font-weight:600; color:#2c3e50;">
					EoI Draft List
				</div>
				<div style="font-size:14px; color:#6c757d; margin:2px 0px; 8px 0">
					Track draft EoIs for your department, monitor key dates, and jump into details quickly.
				</div>
			</div>
		@endif
					...
					<td class="padding-10" colspan="3">
						<div class="table-filters-row">
							<div class="filters-group">
								<select name="pagesize" id="pagesize" class="select2 selectbx" onchange="loadData(1,'{{ route('depteoirequest.html') }}')" data-width="90" data-placeholder="RECORDS">
								...
								<select name="eoi_status" id="eoi_status" class="select2" onchange="loadData(1,'{{ route('depteoirequest.html') }}')" data-width="200" data-placeholder="All Status">
								...
								<span class="input-icon">
									<input type="text" placeholder="Search EoI and press enter" class="nav-search-input selectbx" id="pagesearch" name="pagesearch" autocomplete="off" onchange="loadData(1,'{{ route('depteoirequest.html') }}')" tabindex="<?php echo $t++;?>" />
									<i class="ace-icon fa fa-search nav-search-icon"></i>
								</span>
							</div>
<span style="float:right; display:flex; align-items:center; gap:6px;">
  <input type="checkbox" name="is_cancelled" id="is_cancelled" onchange="loadData(1,'{{ route('depteoirequest.html') }}')">
  <label for="is_cancelled">Cancelled EoI</label>
</span>
						</div>
					</td>
				</tr>
...
$(document).ready(function () {
	loadData(1, '{{ route('depteoirequest.html') }}');
});

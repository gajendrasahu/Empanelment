								...
								<table class="mytable pd-8" border="1" style="text-transform:none!important">
									...
									<tr class="myheadbg">
										<td colspan="6" class="form-label">
											...
											<span><b>Upate Interview Date & Time</b></span>
											<input type="text" class="form-control interview_date" placeholder="dd-mm-YYYY" name="interview_date" id="interview_date" style="width:150px;">	
							...
							<div class="table-responsive">
								<table class="mytable" border="1" style="text-transform:none!important">
									...
									<tr>
										...
										<td class="center padding-10 v-top">@if($vendor->interviewdate){{ date('d\-m\-Y h:i A',strtotime($vendor->interviewdate)) }}@endif</td>
										<td class="padding-10 v-top" nowrap>{{ $vendor->interviewlink }}</td>
										<td class="padding-10 center">
											<a href="{{ route('view.deptparticipations',Crypt::encrypt($vendor->participationid))}}">
...
{
	bootbox.confirm('Do you want to set interview link?',function(result){
		if(result)
		{
			...
			$.ajax({
				url: rl,
				type: 'POST',
				data: {'participationid':participationid,'interviewlink':interviewlink},
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // CSRF token
				},
				success: function (response) {
...
function updateInterviewDate(requestid)
{
	...
	bootbox.confirm('Do you confirm this action?.',function(result){
		...
		{
			$.ajax({
				url: '{{route('updatedept.interviewdate')}}',
				type: 'POST',
				data: {'requestid':requestid,'interviewdate':interviewdate},
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // CSRF token

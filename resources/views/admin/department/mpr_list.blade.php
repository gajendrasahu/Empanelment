	...
	<div class="main-content-inner">
			<div class="page-content">
				...
				<div class="row">
					<div class="col-xs-12">
					<!-- PAGE CONTENT BEGINS -->
<div class="tabbable animated fadeInUp delay-02">
		...
		@if(in_array(2,Session::get('actions')) || in_array(3,Session::get('actions')) || in_array(12,Session::get('actions')))
			<li @if(!in_array(1,Session::get('actions'))) class="active" @endif>
				<a data-toggle="tab" class="form-label font-14" href="#recordlist" onclick="loadData(1,'{{ route('mpr.html') }}')">
	...
	<div class="row">
		...
		<form name="pagedata" id="pagedata" action="#" method="post" onsubmit="return false;">
			...
			<div class="table-responsive">
				<table class="table-bordered pd-10 mytable" style="" border="1">
				<tr>
					<td colspan="8">
						<select name="pagesize" id="pagesize" class="select2" onchange="loadData(1,'{{ route('mpr.html') }}')" data-width="90" data-placeholder="RECORDS">
						...
						<select class="select2" name="vendorid" id="vendorid" onKeyPress="return OnKeyPress(this, event)" onchange="loadData(1,'{{ route('mpr.html') }}');" data-width="175" data-placeholder="Firm Name">
							<option value=""></option>
						...
						<select class="select2" name="orderid" id="orderid" onKeyPress="return OnKeyPress(this, event)" onchange="loadData(1,'{{ route('mpr.html') }}');" data-width="175" data-placeholder="Order Number">
							<option value=""></option>
						...
						<select class="select2" name="month" id="month" onKeyPress="return OnKeyPress(this, event)" onchange="loadData(1,'{{ route('mpr.html') }}');" data-width="175" data-placeholder="All Month">
							<option value=""></option>
							@for($i=1;$i<=12;$i++)
							<option value="{{ $i }}">{{ DateTime::createFromFormat('!m', $i)->format('F'); }}</option>
							@endfor
						</select>
						<select class="select2" name="year" id="year" onKeyPress="return OnKeyPress(this, event)" onchange="loadData(1,'{{ route('mpr.html') }}');" data-width="100">
							@for($i=date('Y');$i>=2025;$i--)
							<option value="{{ $i }}">{{ $i }}</option>
							@endfor
						</select>

						<select class="select2" name="approval" id="approval" onKeyPress="return OnKeyPress(this, event)" onchange="loadData(1,'{{ route('mpr.html') }}');" data-width="175">
				...
				<tr>
					<td colspan="8" style="padding:0px!important;">
						<span class="input-icon" style="width:100%!important;">
							<input type="text" placeholder="Search ..." class="nav-search-input" id="pagesearch" name="pagesearch" autocomplete="off" onchange="loadData(1,'{{ route('mpr.html') }}')" tabindex="<?php echo $t++;?>" @if($search!='')value="{{str_replace('-','/',$search)}}"@endif style="width:100%!important;"/>
...
function viewMprDepartment(r1,mprid)
{
	$.get(""+r1,
	{
		mprid:mprid,

...
<div id="home" class="tab-pane in active" style="padding:0px 10px!important;">
	<div class="row">
			<form name="setdeploymentdate" id="setdeploymentdate" action="{{route('update.deploymentverification',Crypt::encrypt($order->orderid))}}" method="post" enctype="multipart/form-data">
						...
						<div class="ui-card-detail">
							<div>
								...
								<div class="details-detail" style="line-height:25px;">
									...
									Order No : <b>{{ $order->ordernumber }}</b> | Order Date : <b>{{ date('d\-m\-Y',strtotime($order->orderdate)) }}</b><br>
						...
						<div class="section-block-detail">
							...
							<table class="mytable" border="1" style="text-transform: none!important;">
								<tr class="myheadbg">
									...
									<td class="padding-10 font-bold">Name & Employee Code</td>
									<td class="padding-10 font-bold">Mobile Number & Email</td>
									<td class="padding-10 center font-bold width-130">Date of Joining</td>
									<td class="padding-10 center font-bold width-150">Verification Status</td>
									<td class="padding-10 center font-bold width-50"></td>
								</tr>
								@php
								$totalmanmonth		=	0;
								$adminchargetotal	=	0;
								...
								<tr>
									...
									<td class="padding-10 v-top">
										<p>{{ucwords(strtolower($req->sectorname))}}</p><p>{{$req->consultantposition}}</p>{{$req->experience}}
									</td>
									<td class="padding-10 v-top">
										{{$req->employee_code}}<br>{{$req->name}}
									</td>
									<td class="padding-10 v-top">
										{{$req->mobilenumber}}<br>{{$req->email}}
									</td>
									<td class="padding-10 v-top center">
										@if(!is_null($req->deployed_date)){{date('d\-m\-Y',strtotime($req->deployed_date))}}@endif
									</td>
									<td class="padding-10 v-top center">
									{{strtoupper($req->deployment_status)}}
									</td>
									<td class="padding-10 v-top center">

...
<div class="main-content">
	<div class="main-content-inner">
			<div class="page-content" style="padding:10px 15px!important;">
				...
				<div class="row">
					<div class="col-xs-12">
				<form name="frm" id="frm" action="{{ route('store.hiring',0)}}" data-url="{{ route('update.eoirecord')}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="multi-panel-body">
                        ...
                        <div class="multi-form-section" id="step1">
							...
							<div class="col-sm-12 padding-5">
								...
								<textarea class="form-control tinymce" name="projectobjective" id="projectobjective" placeholder="Project objective" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}">{!!old('projectobjective',$eoi->projectobjective)!!}</textarea>										
							...
							<div class="col-sm-12 padding-5">
								...
								<textarea class="form-control tinymce" name="scopeofwork" id="scopeofwork" placeholder="Enter scope of work detail" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}">{{old('scopeofwork',$eoi->scopeofwork)}}</textarea>									
							...
							<div class="col-sm-12 padding-5">
								...
								<textarea class="form-control tinymce" name="anyother" id="anyother" placeholder="Special Condition of Contract (if any)" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}">{{old('anyother',$eoi->anyother)}}</textarea>									
                        ...
                        <div class="multi-form-section active" id="step2" style="height:300px;">
							<div class="col-sm-3 padding-5">
								...
								<select class="form-control" name="categoryid" id="categoryid" required onKeyPress="return OnKeyPress(this, event)"  data-placeholder="Type of Empanelment" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}" onchange="GetTiersWithId(this.value,'{{ route('tiers.list') }}','tierid');">
                        ...
                        <!-- Step 3: Requirements -->
                        <div class="multi-form-section" id="step3" style="height:300px; max-height:300px; overflow-y:scroll;">
							<table class="table table-bordered table-striped table-hover resource-table upload-table eoifiles">
								<tr class="myheadbg">
									<td class="padding-8 form-label" colspan="3" style="vertical-align:middle!important;">
										Required Documents (Uploaded)
									</td>
								</tr>
								<tr>
									<td class="padding-10 upload-input">
							<input type="text" name="authletter" required placeholder="" readonly autocomplete="off" class="form-control full-wdth" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}" value="Authorization Letter*">
									</td>
									
									<td class="padding-10 upload-file" >
							<input type="file" class="form-control" required name="authorizationletter" id="authorizationletter" data-id="{{Crypt::encrypt($eoi->requestid)}}" accept="application/pdf" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}" />
									</td>
									<td class="padding-8 center" style="width:130px;">
										<a href="{{route('view.uploadedfile',Crypt::encrypt($eoi->authorizationletter))}}" target="_blank">
							...
							<table class="table table-bordered table-striped table-hover resource-table upload-table fileattachment committeetable">
								<tr class="myheadbg">
								...
								<tr>
									<td class="padding-10 upload-input">
							<input type="text" name="attachmenttitle{{$loop->iteration}}" id="attachmenttitle{{$loop->iteration}}" required placeholder="Attachment title" autocomplete="off" class="form-control full-wdth" value="{{$attach->attachmenttitle}}" onKeyPress="return OnKeyPress(this, event)" tabindex="{{$t++}}">
									</td>
									<td class="padding-8 center" style="width:250px;">
										<button type="button" class="btn btn-info" style="width:120px;" onclick="updateAttachmentTitle({{$loop->iteration}},'{{Crypt::encrypt($attach->recordid)}}','{{Crypt::encrypt($eoi->requestid)}}')">
											<i class="fa fa-eye"></i> Update Title
										</button>

										<a href="{{route('view.uploadedfile',Crypt($attach->attachmentfile))}}" target="_blank">
										<button type="button" class="btn btn-info" style="width:100px;">
											<i class="fa fa-eye"></i> View File
										</button>
						...
						-->
                            ...
                            <button type="button" class="multi-btn multi-btn-success" id="submitBtn" style="display: none;">Upload New Files</button>
                        </div>
                    </div>
                </div>
...
function editEois(rl,recordid)
{
	...
	$.ajax({
		url: '{{ route("update.resourcedetail") }}',
		method: 'POST',
		data: formData,
		processData: false,
		contentType: false,
...
@if(session('categoryid')==2)
	setTimeout(function() { GetForm('{{route("load-updateform")}}'); },2000);
@endif
*/
setTimeout(function() { GetUpdateForm('{{route("load-updateform")}}'); },2000);
...
function updateAttachmentTitle(ind,recordid,requestid)
{
		...
		$.ajax({
			url: '{{ route("update.attachmenttitle") }}',
			method: 'POST',
			data: formData,
			processData: false,
			contentType: false,
                    ...
                    if(response.fail=='')
                    {
						bootbox.alert({
							...
							callback: function () {
								setTimeout(function () {
									$(".no-skin").css("padding-right", "");
									GetUpdateForm('{{route("load-updateform")}}');
								},1000);
							}
						});
                    }
...
jQuery(function($) {
	...
	$('#submitBtn').on('click', function(e) {
		...
		$.ajax({
			url: '{{ route("upload.attachments") }}',  // Replace with your actual route
			method: 'POST',
			data: formData,
			processData: false,
			contentType: false,

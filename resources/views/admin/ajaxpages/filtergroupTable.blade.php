...
<tr class="mytr" id="item-{{ $i }}">
	...
	<td nowrap style="text-align:left;">{{ $item->filtergroup }}</td>
	...
	@if(in_array(2,Session::get('actions')))
		<a href="{{ route('edit.filtergroup',$item->filtergroupid) }}" title="" class="myactionlink"><i class="fa fa-edit"></i></a></td>
	@endif
    <td nowrap style="text-align:center;">
	@if(in_array(3,Session::get('actions')))
        <a title="" class="myactionlink" onclick="deleteItem('filtergroup',{{ $item->filtergroupid}},{{ $i }},'ARE YOU SURE YOU WANT TO DELETE THIS RECORD?','FILTER GROUP DELETED SUCCESSFULLY')" title=""><i class="fa fa-trash"></i>
        </a>
	@endif
    </td>
</tr>
...
paginationLinks.forEach(link => {
	link.onclick = function(event) {
		...
		const url = '{{ route('filtergroup.html') }}'; // URL generated by Laravel route

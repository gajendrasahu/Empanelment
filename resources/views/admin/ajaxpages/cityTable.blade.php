...
<tr class="mytr" id="item-{{ $i }}">
	...
	<td nowrap>{{ $item->citycode }}</td>
    <td nowrap style="text-align:center;">
	@if(in_array(2,Session::get('actions')))
		<a href="{{ route('edit.city',Crypt::encrypt($item->cityid)) }}" title=""><i class="fa fa-edit"></i></a>
	@endif
	</td>
    <td nowrap style="text-align:center;">
	@if(in_array(3,Session::get('actions')))
        <a title="" onclick="deleteItem('city','{{ Crypt::encrypt($item->cityid)}}',{{ $i }},'{{ __('messages.confirmation') }}','{{ __('messages.deleted') }}')" title=""><i class="fa fa-trash"></i>
        </a>
	@endif
    </td>
</tr>
...
const paginationLinks = document.querySelectorAll('.pagination a');
        paginationLinks.forEach(link => {
            link.onclick = function(event) {
                ...
                const url = '{{ route('city.html') }}'; // URL generated by Laravel route

...
<tr class="mytr" id="item-{{ $i }}">
	...
	@if(in_array(2,Session::get('actions')))
		...
		<a href="{{ route('edit.assettype',Crypt::encrypt($item->assettypeid)) }}" title=""><i class="fa fa-edit"></i></a></td>
		@endif
	@endif
    <td nowrap style="text-align:center;">
	@if(in_array(3,Session::get('actions')))
		@if(Session::get('loginId')===1 || Session::get('loginId')===$item->createdby)
        <a title="" onclick="deleteItem('assettype','{{ Crypt::encrypt($item->assettypeid) }}',{{ $i }},'ARE YOU SURE YOU WANT TO DELETE THIS RECORD?','ASSET TYPE DELETED SUCCESSFULLY')" title=""><i class="fa fa-trash"></i>
        </a>
		@endif
	@endif
    </td>

...
<tr class="mytr" id="item-{{ $i }}">
	...
	<td nowrap>{{ $item->discount }}</td>
	<td nowrap>{{ $item->commission }}</td>
    <td nowrap style="text-align:center;">
	@if(in_array(2,Session::get('actions')))
		<a href="{{ route('edit.tier',Crypt::encrypt($item->tierid)) }}" title=""><i class="fa fa-edit"></i></a></td>
	@endif
    <td nowrap style="text-align:center;">
	@if(in_array(3,Session::get('actions')))
        <a title="" onclick="deleteItem('tier','{{ Crypt::encrypt($item->tierid) }}',{{ $i }},'{{ __('messages.confirmation') }}','{{ __('messages.deleted') }}')" title=""><i class="fa fa-trash"></i>
        </a>
	@endif
    </td>
</tr>
...
const paginationLinks = document.querySelectorAll('.pagination a');
        paginationLinks.forEach(link => {
            link.onclick = function(event) {
                ...
                const url = '{{ route('tier.html') }}'; // URL generated by Laravel route

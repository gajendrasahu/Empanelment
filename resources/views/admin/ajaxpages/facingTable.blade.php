...
<tr class="mytr" id="item-{{ $i }}">
    ...
    <td nowrap>{{ $item->facingname }}</td>
    <td nowrap style="text-align:center;">
	@if(in_array(2,Session::get('actions')))
		@if(Session::get('loginId')===1 || Session::get('loginId')===$item->createdby)
		<a href="{{ route('edit.facing',Crypt::encrypt($item->facingid)) }}" title=""><i class="fa fa-edit"></i></a></td>
		@endif
	@endif
    <td nowrap style="text-align:center;">
	@if(in_array(3,Session::get('actions')))
		@if(Session::get('loginId')===1 || Session::get('loginId')===$item->createdby)
        <a title="" onclick="deleteItem('facing','{{ Crypt::encrypt($item->facingid) }}',{{ $i }},'ARE YOU SURE YOU WANT TO DELETE THIS RECORD?','FACING NAME DELETED SUCCESSFULLY')" title=""><i class="fa fa-trash"></i>
        </a>
		@endif
	@endif
    </td>
...
const paginationLinks = document.querySelectorAll('.pagination a');
        paginationLinks.forEach(link => {
            link.onclick = function(event) {
                ...
                const url = '{{ route('facing.html') }}'; // URL generated by Laravel route

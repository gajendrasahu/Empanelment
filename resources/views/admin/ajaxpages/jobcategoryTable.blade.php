...
<tr class="mytr" id="item-{{ $i }}">
	...
	@if(in_array(2,Session::get('actions')))
		<a href="{{ route('edit.jobcategory',Crypt::encrypt($item->categoryid)) }}" title=""><i class="fa fa-edit"></i></a></td>
	@endif
    <td nowrap style="text-align:center;">
	<!--
	@if(in_array(3,Session::get('actions')))
        <a title="" onclick="deleteItem('jobcategory','{{ Crypt::encrypt($item->categoryid) }}',{{ $i }},'{{ __('messages.confirmation') }}','{{ __('messages.deleted') }}')" title=""><i class="fa fa-trash"></i>
        </a>
	@endif
	-->
    </td>
...
const paginationLinks = document.querySelectorAll('.pagination a');
        paginationLinks.forEach(link => {
            link.onclick = function(event) {
                ...
                const url = '{{ route('jobcategory.html') }}'; // URL generated by Laravel route

...
<tr class="mytr" id="item-{{ $i }}">
	...
	<td nowrap style="text-align:center;">{{ date('d\-m\-Y',strtotime($item->purchasedate)) }}</td>
	<td nowrap style="text-align:center;">{{ $item->billnumber }}</td>
	...
	<td nowrap style="text-align:center;">
		@if($item->balanceamount>0)
			<span style="color:red; font-weight:bold;">{{ number_format($item->balanceamount,'2','.','') }}</span>
		@else
	...
	@if(in_array(2,Session::get('actions')))
		<a href="{{ route('change.purchase',$item->purchaseid) }}" title="" class="myactionlink"><i class="fa fa-edit"></i></a></td>
	@endif
    <td nowrap style="text-align:center;">
	@if(in_array(3,Session::get('actions')))
        <a title="" class="myactionlink" onclick="deleteItem('purchase',{{ $item->purchaseid}},{{ $i }},'ARE YOU SURE YOU WANT TO DELETE THIS RECORD?','PURCHASE RECORD DELETED SUCCESSFULLY')" title=""><i class="fa fa-trash"></i>
        </a>
	@endif
    </td>
    <td nowrap style="text-align:center;">
	@if(in_array(4,Session::get('actions')))
        <a href="{{ route('print.purchase', $item->purchaseid) }}" target="_blank" title="" class="myactionlink" title=""><i class="fa fa-print"></i></a>
	@endif
    </td>
    <td nowrap style="text-align:center;">
	@if(in_array(20,Session::get('actions')))
        <a href="{{ route('add.purchasepayment', $item->purchaseid) }}"><i class="fa fa-money" title="ADD PAYMENT"></i></a>
	@endif
    </td>
</tr>
@endforeach

<div class="modal-body" style="padding:10px 20px;">
	<div class="table-responsive printableResourceData">
		<table class="table table-bordered font-12">
			<thead>
				<tr>
					<td colspan="2" nowrap>Department / Project Manager</td>
				...
				<tr>
					<td colspan="7" class="table-label">Sector : {{ucwords(strtolower($sectorname))}} | {{$data->tiername}}</td>
				</tr>
			</thead>
			<tbody>
				@foreach($positions as $position)
				<td colspan="7" class="table-label-fade font-bold">
					Position : {{$position->consultantposition}} | Remuneration : INR {{$position->remuneration}}
				</td>
				...
				@foreach($position->resources as $resource)
				<tr>
					<td nowrap class="text-center width-30">{{$loop->iteration}}</td>
					<td nowrap class="width-100">{{$resource->employee_code}}</td>
					<td nowrap>{{$resource->name}}</td>
					<td nowrap class="width-100">{{$resource->mobilenumber}}</td>
					<td nowrap class="width-100">{{$resource->email}}</td>
					<td nowrap class="text-center width-100">@if($resource->deployed_date) {{date('d\-m\-Y',strtotime($resource->deployed_date))}} @endif</td>

...
<div class="tabbable animated fadeInUp delay-02" style="padding:0px 0px!important;">
			<div class="eoi-section-header" onclick="loadData('{{ route('undeployedresource.html') }}')" style="cursor:pointer; padding:8px 8px 16px 8px; position:relative;">

				<div style="font-size:20px; font-weight:600; color:#2c3e50;">
					Undeployed Resources
				</div>
				<div style="position:absolute; right:5px; top:0px;">
						<button type="button" class="btn btn-info gridbtn" style="width:100px; padding:0px!important;" onClick="loadData('{{ route('undeployedresource.html') }}')" style="width:120px;">
			...
			<div class="table-responsive">
				...
				<tr>
					<td style="padding:5px!important; font-weight:600;">
						...
						<select class="select2" name="categoryid" id="categoryid" onKeyPress="return OnKeyPress(this, event)"onchange="getVendors(this.value); loadData('{{ route('undeployedresource.html') }}')" data-width="190" data-placeholder="Category Name">
						...
						<select class="select2" name="managerid" id="managerid" onKeyPress="return OnKeyPress(this, event)" data-width="190" data-placeholder="Project Manager">
							<option value=""></option>
				...
				<tr>
					<td style="position:relative; padding:5px!important; font-weight:600;">
						...
						<select name="level_id" id="level_id" class="select2 selectbx" onchange="loadData('{{ route('undeployedresource.html') }}')"  data-width="190" data-placeholder="Experience Level">
							<option value=""></option>
							@foreach($levels as $lvl)
							<option value="{{$lvl->experiencelevel}}">Level-{{$lvl->experiencelevel}}</option>
							@endforeach
						</select>
						<select name="format_id" id="format_id" class="select2 selectbx" onchange="loadData('{{ route('undeployedresource.html') }}')"  data-width="190" data-placeholder="Report Format">
							<option value="1">Report Format - 1</option>
						</select>

						<span class="input-icon" style="position:absolute; top:2px; width:190px!important;">
							<input type="text" placeholder="Search ..." class="nav-search-input" id="pagesearch" name="pagesearch" autocomplete="off" onchange="loadData('{{ route('undeployedresource.html') }}')" tabindex="<?php echo $t++;?>" style="width:100%!important;" />
							<i class="ace-icon fa fa-search nav-search-icon" style="margin-top:0px;"></i>
						</span>

<span>
  <input type="checkbox" name="in_house" id="in_house" onchange="loadData('{{ route('undeployedresource.html') }}')" style="vertical-align:text-top;">
...
<script>
    var resourceReportUrl 	= "{{ route('undeployed.report') }}";
	var resourceAwdReportUrl= "{{ route('awdundeployed.report') }}";
...
function clearData()
{
	...
	$('#experiencelevel').val('').trigger('change');
	$('#pagesearch').val('');
	setTimeout(function() { loadData('{{ route('undeployedresource.html') }}'); },1000);
}
$(document).ready(function() {
	...
	loadData('{{ route('undeployedresource.html') }}');
});

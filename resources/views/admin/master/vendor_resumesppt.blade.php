					...
					<div class="content-detail">
						<div class="ui-card-detail">
							<div>
								...
								<div class="details-detail lh-25">
									<b>Department / Project Manager :</b> {{ $eoi->departmentname }}<br>
						...
						<div class="section-block-detail">
							...
							<table class="mytable" border="1" style="text-transform: capitalize!important;">
								...
								@if($eoi->categoryid==2)
								<tr class="myheadbg">
									<td class="mytd form-label padding-10" nowrap style="position:relative;">{{$index+1}}) Sector : {{$req->sectorname}} | Position : {{$req->consultantposition}} | Experience <i class="fa fa-info-circle" title="As per empanelment"></i> : {{$req->experience}} | Duration : {{$req->duration}} Months
									
									@if($req->isAdditional==1)
									<span class="bg-info font-bold" style="position:absolute; right:5px; top:5px; padding:2px 5px; border-radius:5px;"><i class="fa fa-angle-double-right"></i> Additional Resource</span>
									@endif
									</td>
								</tr>
								@else
								<tr class="myheadbg">
									<td class="mytd form-label padding-10" nowrap>{{$index+1}}) Role : {{$req->role}} | Level : {{$req->experiencelevel}} | Experience <i class="fa fa-info-circle" title="As per empanelment"></i> : {{$req->experience}} | Duration : {{$req->duration}} Months</td>
								</tr>
								@endif
								@if($req->qualification)
								<tr class="myheadbg">
									<td class="mytd form-label padding-10">Qualification : {!!$req->qualification!!}</td>
								</tr>								
								@endif
								@if($req->resumes->count()==0)
								<tr class="">
									<td class="mytdwhite padding-5" style="width:20px;" nowrap></td>
								</tr>
								@endif
								@if($req->resumes->count()>0)
								<tr>
									<td class="padding-0">
										<table class="mytable" border="1" style="text-transform: capitalize!important;">
											<tr class="">
												...
												<td class="padding-10" style="width:100px;">&nbsp;</td>
											</tr>
											@foreach($req->resumes as $resume)
											<tr>
												<td class="padding-10">
													@if($resume->isresumeviewed==0)
													<a href="{{route('view.uploadedfile',Crypt::encrypt($resume->resume))}}" data-id="{{Crypt::encrypt($resume->resumeid)}}" target="_blank" class="viewresumebtn btn btn-info width-100 no-hover">
														<i class="fa fa-file-pdf-o"></i> View
													</a>
													@else
													<a href="{{ asset('storage/' . $resume->resume) }}" data-id="{{Crypt::encrypt($resume->resumeid)}}" target="_blank" class="viewresumebtn btn btn-info width-100 no-hover">
														<i class="fa fa-file-pdf-o"></i> Viewed
													</a>
													@endif
												</td>
												...
												<td class="padding-10">
													<input type="text" name="remark{{$resume->resumeid}}" id="remark{{$resume->resumeid}}" value="{{$resume->remark}}" style="width:100%;" placeholder="Update remark" class="">
												</td>				
												<td class="padding-10">
													@if($resume->isresumeviewed==1)
													<button type="button" class="btn btn-info gridbtn" onclick="UpdateRemark('{{route('update.resumeremark')}}',{{$resume->resumeid}})" style="width:130px;"><i class="fa fa-refresh"></i> Update Remark</button>
													@else
													<button type="button" class="btn btn-info gridbtn" disabled style="width:130px;" title="Resume must be reviewed to update remark.">
														<i class="fa fa-refresh"></i> Update Remark
													</button>
													@endif
												</td>
											</tr>
							...
							<table class="" style="text-transform: none!important; margin-top:20px; width:100%;">
								<tr>
									...
									@if($floated->presentationfile!='')
										<a href="{{route('view.uploadedfile',Crypt::encrypt($floated->presentationfile))}}" target="_blank">
											<span class="btn btn-info gridbtn" style="width:170px;"><i class="fa fa-download"></i> View / Download PPT</span>
										</a>		
									@endif		
									@if($floated->signedcopyofeoi!='')
										<a href="{{route('view.uploadedfile',Crypt::encrypt($floated->signedcopyofeoi))}}" target="_blank">
											<span class="btn btn-info gridbtn" style="width:250px;"><i class="fa fa-download"></i> View / Download Signed EoI File</span>
										</a>		
									@endif		
									</td>
								</tr>
								...
								<tr>
									<td class="padding-0 right">
										...
										<button type="button" class="btn btn-info gridbtn" onclick="UpdateParticipationRemark('{{route('update.participationremark')}}',{{$participation->participationid}})" style="width:130px;"><i class="fa fa-refresh"></i> Update Remark</button>
...
function UpdateRemark(rl,resumeid)
{
	bootbox.confirm('Do you want to update this remark?',function(result){
		if(result)
		{
			var remark	=	$("#remark"+resumeid).val();
			$.ajax({
				url: rl,
				type: 'POST',
				data: {'resumeid':resumeid,'remark':remark},
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // CSRF token
				},
				success: function (response) {
					...
					{
						bootbox.alert({
							message: "Remark and status updated successfully",
							callback: function () {
					...
					{
						bootbox.alert({
							message: '<span style="color:red; font-size:14px;">'+response.message+'</span>',
							callback: function () {
								setTimeout(function() { $(".no-skin").css("padding-right",""); },500);
								$("#reviewstatus"+resumeid).val('0');
							}
						});						
					}
				},
...
function UpdateParticipationRemark(rl,participationid)
{
	bootbox.confirm('Do you want to update this remark?',function(result){
		if(result)
		{
			...
			$.ajax({
				url: rl,
				type: 'POST',
				data: {'participationid':participationid,'remark':remark},
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // CSRF token
				},
				success: function (response) {
...
$('.viewresumebtn').on('click', function(e)
{
	...
	var form = $('#committeefrm')[0];
	var formData = new FormData(form);
	formData.append('_token', $('meta[name="csrf-token"]').attr('content'));
	formData.append('resumeid',resumeid);
	$.ajax({
		url: '{{route("setresume.status")}}',
		method: 'POST',
		data: formData,
		processData: false,
		contentType: false,

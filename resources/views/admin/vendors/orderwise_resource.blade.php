...
<div class="tabbable animated fadeInUp delay-02" style="padding:0px 0px!important;">
			<div class="eoi-section-header" onclick="loadData('{{ route('vendororderwiseresource.html') }}')" style="cursor:pointer; padding:8px 8px 16px 8px; position:relative;">
			...
			<div class="table-responsive">
				...
				<tr>
					<td style="position:relative; padding:5px!important;">
						...
						<select class="select2" name="departmentid" id="departmentid" onKeyPress="return OnKeyPress(this, event)" onchange="loadData('{{ route('vendororderwiseresource.html') }}');" data-width="250" data-placeholder="@if($vendor->categoryid==2) Department / Project Manager @else Project Manager @endif">
							<option value=""></option>
						...
						<select name="sectorid" id="sectorid" class="select2 selectbx" onchange="loadData('{{ route('vendororderwiseresource.html') }}')"  data-width="200" data-placeholder="Sector">
							<option value=""></option>
							@foreach($sectors as $sector)
							<option value="{{$sector->sectorid}}">{{$sector->sectorname}}</option>
							@endforeach
						</select>
						<select name="positionid" id="positionid" class="select2 selectbx" onchange="loadData('{{ route('vendororderwiseresource.html') }}')"  data-width="200" data-placeholder="Position">
							<option value=""></option>
						...
						<select name="level_id" id="level_id" class="select2 selectbx" onchange="loadData('{{ route('vendororderwiseresource.html') }}')"  data-width="200" data-placeholder="Experience Level">
							<option value=""></option>
							@foreach($levels as $lvl)
							<option value="{{$lvl->experiencelevel}}">Level-{{$lvl->experiencelevel}}</option>
							@endforeach
						</select>
						<input type="hidden" name="sectorid" id="sectorid" value="0">
						<input type="hidden" name="positionid" id="positionid" value="0">
						@endif

						<select name="order_type" id="order_type" class="select2 selectbx" onchange="loadData('{{ route('vendororderwiseresource.html') }}')"  data-width="200" data-placeholder="All Order">
						...
						<button type="button" class="btn btn-info gridbtn" onClick="loadData('{{ route('vendororderwiseresource.html') }}')" style="position:absolute; right:5px; width:120px;">
				...
				<tr>
					<td style="padding:5px!important;">
						<span class="input-icon" style="width:100%!important; margin-top:0px;">
							<input type="text" placeholder="Search ..." class="nav-search-input" id="pagesearch" name="pagesearch" autocomplete="off" onchange="loadData('{{ route('vendororderwiseresource.html') }}')" tabindex="<?php echo $t++;?>" style="width:100%!important;" />
...
$(document).ready(function() {
	...
	loadData('{{ route('vendororderwiseresource.html') }}');
});
			...
			$.ajax({
				...
				success: function(response){
					...
					if(response.status==200)
					{
						...
						setTimeout(function(){ loadData('{{ route('vendororderwiseresource.html') }}'); CloseThis() },2000);
					}
					else
					{
						bootbox.alert(response.message);

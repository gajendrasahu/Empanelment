...
<table class="order-table pd-8">
    ...
    <!-- ORDER SUMMARY -->
    <tr>
        <td colspan="@if($rec->categoryid==2) 9 @else 8 @endif" class="order-summary">
    ...
    <tr class="resource-row">
        <td class="center width-30" style="position:relative;">
			...
			<a href="{{ route('workordervendor.replaceresource', Crypt::encrypt($resource->deploymentid)) }}">
			<span class="badge-info" style="background-color:#147E8B; font-size:10px!important; position:absolute; left:0; bottom:0; width:100%; color:white; border-radius:4px 4px 0px 0px;">
				Replace
			</span>
			</a>
			@endif
        ...
        <td style="padding:0px!important; width:100px!important;" class="text-center">
			...
			@elseif($resource->deployment_status=='Released')
				Released
			@elseif($resource->deployment_status=='Active' || $resource->deployment_status=='Pending')
				<button type="button" class="btn btn-success updateResource gridbtn" style="padding:4px; height:35px; border:none!important; background-color:#147E8B!important;">Update</button>

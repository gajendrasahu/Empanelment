			...
			<form id="frmVendorPayment" method="POST" action="{{ route('finance.vendorpayment.store') }}" enctype="multipart/form-data">
					...
					<div class="card-body mt-10">
						...
						<div class="row">
							...
							<div class="col-md-4 mb-3">
								...
								<select name="tds_tax_id" id="tds_tax_id" class="form-control" onchange="calculateVendorPayment()" required>
				<option value="">---</option>
				@foreach($tdsTaxes as $tax)
				
				<option value="{{ $tax->rate }}" data-rate="{{ (float)$tax->rate }}" {{ $defaultTdsTaxId == $tax->tax_id ? 'selected' : '' }}>
				...
				@endforeach
							...
							<div class="col-md-4 mb-3">
								...
								<select name="gst_tds_tax_id" id="gst_tds_tax_id" class="form-control" onchange="calculateVendorPayment()" required>
								<option value="">---</option>
								@foreach($gstTdsTaxes as $tax)
								<option value="{{ $tax->rate }}" data-rate="{{ (float)$tax->rate }}" {{ $defaultGstTdsTaxId == $tax->tax_id ? 'selected' : '' }}>
								{{ $tax->tax_name }} ({{ number_format((float)$tax->rate,2) }}%)
								</option>
								@endforeach
								</select>
...
function calculateVendorPayment()
{
	...
	let tdsRate = Number(document.getElementById('tds_tax_id').value) || 0;
	let tdsAmount = 0;
	let gstTdsRate = Number(document.getElementById('gst_tds_tax_id').value) || 0;
	let gstTdsAmount = 0;
...
$(document).on('click','#btnSubmitPayment',
	function()
	{
		...
		let tdsTaxId = $('#tds_tax_id').val();
		let gstTdsTaxId = $('#gst_tds_tax_id').val();

		if(tdsTaxId == '')
		{
			bootbox.alert('Please select TDS.');
			$('#tds_tax_id').focus();
			return false;
		}
		...
		if(gstTdsTaxId == '')
		{
			bootbox.alert('Please select GST-TDS.');
			$('#gst_tds_tax_id').focus();
			return false;
		}

...
class VendorManagementController extends Controller
{
    ...
    public function storeInvoiceVoucher(Request $request)
	{
        ...
        $messages = [
			...
			'tax_type.in'       		=> 	'Tax type must be either INTRA_STATE or INTER_STATE.',
        ];
		...
		try
		{
			...
			$operatingcost	=	round((($total*$pricing->operatingmargin)/100),2);
			$taxableAmount	=	$total+$operatingcost;
			
			$gstTax			=	DB::table('finance_tax_master')
								->where('rate',$order->tax)
								->where('is_active',1)
								->where('tax_type','GST')
								->first();
			
			$gstRate		=	(float)$order->tax;
			$gstAmount		=	round(($taxableAmount * $gstTax->rate) / 100,2);
			...
			if($validatedData['tax_type'] == 'INTRA_STATE')
			{
				$cgstPercent	=	round($gstTax->rate / 2,2);
				$sgstPercent	=	round($gstTax->rate / 2,2);
			...
			DB::table('invoice_mpr')->insert([
				'finance_project_id'		=>	$order->projectid ?? NULL,
				...
				'mpr_approved_amount'		=>	$total,
				'operating_margin'			=>	$pricing->operatingmargin ?? 0,
				'operating_cost'			=>	$operatingcost ?? 0,
				'taxable_amount'			=>	$taxableAmount,
				'tax_id'					=>	$gstTax->tax_id ?? NULL,
				...
				'net_invoice_value'			=>	$taxableAmount+$cgstAmount+$sgstAmount+$igstAmount,
				'net_balance_value'			=>	$taxableAmount+$cgstAmount+$sgstAmount+$igstAmount,
				...
				'admin_charge_percent'		=>	$pricing->admincharge ?? 0,
				'admin_charge_gst_percent'	=>	$pricing->tax ?? 0,
			]);
			
			DB::table('mpr_tbl')
			->whereIn('mpr_id',$mpr_ids)
			->update([
				'is_invoiced'	=>	1
			]);
			
			$url = route('vendor.approved.mprs');
    ...
    public function uploadMprDocumentForm(Request $request)
	{
		...
		{
			$mprs	=	DB::table('mpr_tbl as a')
						->select('a.*','b.ordernumber','c.project_name','b.categoryid','d.engagementname')
						->join('eoi_work_order as b','b.orderid','=','a.order_id')
						->leftJoin('project_tbl as c','c.projectid','=','b.projectid')
						->leftJoin('eoi_request as d','d.requestid','=','b.requestid')
						->where('a.vendor_id',Session::get('vendorId'))
						->where('a.is_verified','=',1)
						->where('a.is_invoiced','=',0)
	...
	{
		$orderDetail	=	DB::table('eoi_work_order')->where('orderid',Crypt::decrypt($request->orderid))->first();
		if(!$orderDetail)
		{
		...
		if($orderDetail->categoryid==2)
		{
			$rules['group_mpr_file']	= 	'required|file|mimes:pdf|max:20480';
		}
		else
		{
			$rules['group_mpr_file']	= 	'nullable|file|mimes:pdf|max:20480';
		}
        ...
        $messages = [
			...
			'group_mpr_file.required' 	=> 	'Grouped MPR file is required',
			'group_mpr_file.file' 		=> 	'Invalid grouped MPR file selected',
			'group_mpr_file.mimes' 		=> 	'Invalid MPR file selected',
			'group_mpr_file.max' 		=> 	'Maximum grouped mpr file size of 20 MB is allowed',
		...
		try
		{
			...
			else
			{
				$pendingFiles	=	DB::table('mpr_attendance_summary')
									->where(function ($query) {
										$query->whereNull('attendance_file');
									})
									->where('mpr_id',$validatedData['mprid'])
									->exists();				
			}
			...
			if($exists)
			{
				...
				$group_mpr_file	=	$request->file('group_mpr_file') ?? NULL;
				...
				if($group_mpr_file!='')
				{
					$group_mpr_file	=	$request->file('group_mpr_file')->store('uploads/mprfiles','public');
				}
				if($supporting_file!='')
				{
...
public function storeMprAttendance(Request $request)
{
	...
	$orderDetail	=	DB::table('eoi_work_order')
						->where('orderid',Crypt::decrypt($request->orderid))
						->first();
	...
	if(!$orderDetail)
	{
		return response()->json([
			'status'  => 400,
			'message' => 'Invalid work order detail provided.'
		]);
	}
    ...
    $rules = [
        ...
        'count_days'     =>	'required|numeric|min:0',
        'per_day_salary' =>	'required|numeric|min:0',

        'presents'       =>	'required|numeric|min:0',
        'absents'        =>	'required|numeric|min:0',
        'leaves'         =>	'required|numeric|min:0',
    ...
    $mprExists	=	DB::table('mpr_tbl')
					->where('mpr_id', $request->mprid)
					->first();
	...
	{
		$message	=	NULL;
		if($mprExists->mpr_status=='Completed' || $mprExists->mpr_status=='Verified')
		{
			if($mprExists->mpr_status=='Completed')
			{
				$message	=	'<b>This attendance detail has already been submitted. You cannot update it now.</b>';
			}
			else
			{
				$message	=	'<b>This attendance detail has already been verified. You cannot update it now.</b>';
			}
            return response()->json([
                'status'	=>	400,
                'message'	=> 	$message
		...
		if(!empty($summary->attendance_file))
		{
			$rules['attendancefile'] = 'nullable|file|mimes:pdf|max:5120';
		}
		else
    ...
    */
    $messages = [
        ...
        'month.required'         => 'Month value is required',
        'month.numeric'          => 'Invalid month value',
        'month.between'          => 'Invalid month value',
        ...
        'count_days.required'    => 'Count days value is required',
        'count_days.numeric'     => 'Invalid count days value',

        'presents.required'      => 'Present value is required',
        'presents.numeric'       => 'Invalid present value',
        'presents.min'           => 'Invalid present value',

        'absents.required'       => 'Absent value is required',
        'absents.numeric'        => 'Invalid absent value',
        'absents.min'             => 'Invalid absent value',

        'leaves.required'        => 'Leave value is required',
        'leaves.numeric'         => 'Invalid leave value',
        'leaves.min'             => 'Invalid leave value',

        'holidays.required'      => 'Holiday value is required',
        'holidays.numeric'       => 'Invalid holiday value',
        'holidays.min'           => 'Invalid holiday value',
    ...
    try {
        ...
        $monthStart	=	Carbon::create($validatedData['year'],$validatedData['month'],1)->startOfDay();
        ...
        if(!$deployment)
		{
            ...
            return response()->json([
                'status'  => 400,
                'message' => 'Invalid deployment detail provided.'
            ]);
        }
        ...
        if(!$deployment->deployment_date)
		{
            ...
            return response()->json([
                'status'  => 400,
                'message' => 'Deployment joining date is not available.'
            ]);
        }

        $joiningDate=	Carbon::parse($deployment->deployment_date)->startOfDay();
        $start 		= 	$monthStart->copy();

        if($joiningDate->greaterThan($start))
		{
        ...
        if(!empty($deployment->lastdate))
		{
            $releaseDate = Carbon::parse($deployment->lastdate)->startOfDay();

            if($releaseDate->lessThan($end))
			{
                $end = $releaseDate->copy();
            }
        }

        $order	=	DB::table('eoi_work_order')
					->where('orderid', $deployment->orderid)
					->first();
        ...
        if(!$order)
		{
            ...
            return response()->json([
                'status'  => 400,
                'message' => 'Invalid order detail provided. Please check and try again!'
            ]);
        }

        if(!empty($order->workorderduedate))
		{
            $workOrderDueDate = Carbon::parse($order->workorderduedate)->startOfDay();

            if($workOrderDueDate->lessThan($end))
			{
                $end = $workOrderDueDate->copy();
            }
        }
        ...
        if($start->greaterThan($end))
		{
            ...
            return response()->json([
                'status'  => 400,
                'message' => 'No active deployment days are available for the selected month.'
            ]);
        }

        $total_days = $monthStart->daysInMonth;

        $count_days = $start->diffInDays($end) + 1;
        ...
        $holidayRecords = DB::table('holiday_tbl')
        ...
        */
        foreach ($holidayRecords as $holiday) {
            ...
            */
            if (
                ...
                $holidayStart = Carbon::create(
                    $start->year,
                    $originalStart->month,
                    $originalStart->day
                )->startOfDay();

                $holidayEnd = Carbon::create(
                    $start->year,
                    $originalEnd->month,
                    $originalEnd->day
                )->startOfDay();
        ...
        |--------------------------------------------------------------------------
        */
        $holidayDates = array_values(
            array_unique($holidayDates)
        );

        /*
        |--------------------------------------------------------------------------
        | SERVER HOLIDAY COUNT
        |--------------------------------------------------------------------------
        */
        $serverHolidayCount =
            count($holidayDates);
        ...
        */
        $postedTotalDays =
            (int) $request->input('total_days');

        $postedCountDays =
            (int) $request->input('count_days');

        $presents =
            (int) $request->input('presents', 0);

        $absents =
            (int) $request->input('absents', 0);

        $leaves =
            (int) $request->input('leaves', 0);

        $holidays =
            (int) $request->input('holidays', 0);

        /*
        |--------------------------------------------------------------------------
        | VALIDATE TOTAL MONTH DAYS
        |--------------------------------------------------------------------------
        */
        if ($postedTotalDays != $total_days) {
            ...
            return response()->json([
                ...
                'message' =>
                    "Invalid total days. " .
                    "Expected {$total_days} days for the selected month."
            ]);
        }
        ...
        */
        if ($postedCountDays != $count_days) {
            ...
            return response()->json([
                'status'  => 400,
                'message' =>
                    "Invalid countable days. " .
                    "Expected {$count_days} active days."
            ]);
        }
        ...
        |--------------------------------------------------------------------------
        |
        | H must exactly equal server calculated holidays.
        |--------------------------------------------------------------------------
        */
        if ($holidays != $serverHolidayCount) {
            ...
            return response()->json([
                ...
                'message' =>
                    ...
                    "Expected {$serverHolidayCount} holidays for the selected deployment period."
            ]);
        }
        ...
        */
        ...
        if ($attendanceTotal != $count_days) {
            ...
            return response()->json([
                'status' => 400,
                'message' =>
                    "Total available days for the selected month is {$count_days}. " .
                    "Please ensure that the sum of P, A, L, and H equals {$count_days}."
            ]);
        }
        ...
        */
        ...
        if (!$mprRecord) {
            ...
            $report_month = Carbon::create(
                $validatedData['year'],
                $validatedData['month']
            )->endOfMonth()->toDateString();

            $mprid = DB::table('mpr_tbl')
                ->insertGetId([
        ...
        |--------------------------------------------------------------------------
        */
        $mprResourceExists = DB::table('mpr_resource')
            ->where(
                'deployment_id',
        ...
        */
        $resource = DB::table('eoi_resource_deployment as a')
            ...
            ->leftJoin(
                'remuneration_tbl as d',
                function ($join) use ($vendor, $rateid) {
        ...
        if (!$resource) {
            ...
            return response()->json([
                'status'  => 400,
                'message' => 'Resource remuneration detail not found.'
            ]);
        }
        ...
        */
        ...
        $resource->operating_value =
            (
                $taxable *
                $pricing->operatingmargin
            ) / 100;
        ...
        */
        ...
        if ($request->hasFile('attendancefile')) {
            ...
            $attendancefile =
                $request->file('attendancefile')
                    ->store(
                        'uploads/mprattendancefiles',
                        'public'
                    );
        }
        ...
        if ($request->hasFile('supportingfile')) {
            ...
            $supportingfile =
                $request->file('supportingfile')
                    ->store(
                        'uploads/mprsupportingfiles',
                        'public'
                    );
        }
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        $summaryExists = DB::table('mpr_attendance_summary')
            ->where(
                'mpr_id',
                $validatedData['mprid']
            )
        ...
        if (!$summaryExists) {
            ...
            DB::table('mpr_attendance_summary')->insert([
                ...
                */
                ...
                'engagement' =>
                    $validatedData['engagement'] ?? 0,

                'mpr_remark' =>
                    $validatedData['mpr_remark'] ?? null,
            ...
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | Update uploaded files only if supplied
            |--------------------------------------------------------------------------
            */
            if ($attendancefile) {

                $summaryUpdate['attendance_file'] =
                    $attendancefile;
            }

            if ($mprfile) {

                $summaryUpdate['mpr_file'] =
                    $mprfile;
            }

            if ($supportingfile) {

                $summaryUpdate['supporting_file'] =
                    $supportingfile;
            }
	...
	{	
		...
		$result = DB::select("
			SELECT 
				...
				CASE
					...
					/* -----------------------------------------
					 * After last date
					 * Only apply when lastdate exists
					 * ----------------------------------------- */
					WHEN ? IS NOT NULL
						 AND DATE_ADD(
								DATE(CONCAT(?, '-', LPAD(?, 2, '0'), '-01')),
					...
					WHEN EXISTS (
						SELECT 1
						FROM holiday_tbl h
						WHERE h.is_recurring = 1
						  AND h.recurring_type = 'yearly'
						  AND MONTH(
								DATE_ADD(
									DATE(CONCAT(?, '-', LPAD(?, 2, '0'), '-01')),
									INTERVAL n.n DAY
						  ...
						  AND DAY(
							  ...
							  ) BETWEEN DAY(h.start_date) AND DAY(h.end_date)
					)
					THEN 'H'


					/* -----------------------------------------
					 * Monthly recurring holiday
					 *
					 * Example:
					 * Every month 10th to 12th
					 * ----------------------------------------- */
					WHEN EXISTS (
						SELECT 1
						FROM holiday_tbl h
						WHERE h.is_recurring = 1
						  AND h.recurring_type = 'monthly'
						  AND DAY(
								DATE_ADD(
									DATE(CONCAT(?, '-', LPAD(?, 2, '0'), '-01')),
									INTERVAL n.n DAY
		...
		", [
			...
			/* yearly recurring holiday - month */
			$year, $month,

			/* yearly recurring holiday - day */
			$year, $month,
		...
		$html	=	view('admin.vendors.ajaxpages.loadResourceMpr',[
						'resource'	=>	$resource,
						'order'		=>	$order,
						'vendor'	=>	$vendor,
						'mprid'		=>	$mprid,
	...
	public function loadMprResources(Request $request)
	{
		...
		// First and last date of the selected MPR month
		$startDate = Carbon::createFromDate($year, $month, 1)
						   ->startOfMonth()
						   ->toDateString();

		$lastDate = Carbon::createFromDate($year, $month, 1)
						  ->endOfMonth()
						  ->toDateString();
		...
		$order = DB::table('eoi_work_order as a')
					->select(
						'a.orderid',
						'a.project_name as projecttitle',
						'a.categoryid',
						'a.vendorid',
						'b.ispm'
					)
		...
		try {
			...
			// Get remuneration rate applicable for selected MPR month
			$rateid = DB::table('remuneration_rate_list')
			...
			$resources = DB::table('eoi_resource_deployment as a')
						...
						->whereIn(
							'a.deployment_status',
							['Active', 'Released', 'Extended']
						)

						// Resource must have been deployed on or before
						// the last day of the selected MPR month
						->where(
							'a.deployed_date',
							'<=',
							$lastDate
						...
						// Resource must not have been released before
						// the beginning of the selected MPR month
			...
			$html = view(
				'admin.vendors.ajaxpages.loadMprResources',
				[
					'resources' => $resources,
					'order'     => $order,
					'mprid'     => $mprid,
		...
		{
			Log::error('Error in loadMprResources: ' . $e->getMessage());
			return response()->json([
				'status' => 500,
    ...
    public function loadMprMonths(Request $request)
	{
		...
		try
		{
			...
			*/
			...
			$default	=	Carbon::create(2026, 5, 1, 0, 0, 0);
    ...
    public function vendorPendingMpr(Request $request)
	{
        Session::put('adminmenu','mprmanagament');
		Session::put('adminsubmenu','pendingmprs');
		Session::put('menid',176);
		$userId		= $request->session()->get('userId');
		$issuper	= $request->session()->get('issuper');

		return view('admin/vendors/vendor_pending_mprs');
    }


	public function vendorPendingMprData(Request $request)
	{
		$userId     = $request->session()->get('userId');
		$vendorId   = $request->session()->get('vendorId') ?? 0;
		$subUserId  = $request->session()->get('subUserId') ?? 0;

		$mpr_month  = $request->input('mpr_month');
		$mpr_year   = $request->input('mpr_year');
		$mpr_type   = $request->input('mpr_status');

		$pagesearch  = trim($request->input('pagesearch', ''));
		...
		$defaultMprStartDate = Carbon::create(2026, 6, 1)->startOfMonth();
		...
		$orders = DB::table('eoi_work_order as a')
			...
			->leftJoinSub(
				DB::table('finance_opening_balance')
					->select(
						'eoi_number',
						DB::raw('MAX(opening_date) as opening_date')
					)
					->groupBy('eoi_number'),
				'f',
				function ($join) {
					$join->on('f.eoi_number', '=', 'c.eoinumber');
				}
			)
			->when($vendorId > 0, function ($query) use ($vendorId) {
				$query->where('a.vendorid', $vendorId);
			})
			->when($subUserId > 0, function ($query) use ($subUserId) {
				$query->where('a.subuserid', $subUserId);
			})
			->when($pagesearch !== '', function ($query) use ($pagesearch) {
				$query->where(function ($q) use ($pagesearch) {
					$q->where('a.ordernumber', 'like', '%' . $pagesearch . '%')
						...
						->orWhere('c.eoinumber', 'like', '%' . $pagesearch . '%');
				});
			})
			->orderBy('a.orderdate')
			->get();
		...
		foreach ($orders as $order)
		{
			...
			{
				$mprStartDate	=	Carbon::parse($order->opening_date)->startOfMonth();
			}
			else
			{
				$mprStartDate = $defaultMprStartDate->copy();
			}
			$deployment = DB::table('eoi_resource_deployment')
				->where('orderid', $order->orderid)
				->whereNotNull('deployed_date')
			...
			if(!$deployment)
			{
				$order->months = collect();
				continue;
			}

			$startDate = Carbon::parse($deployment->deployed_date);
			$orderStartDate = Carbon::parse($order->orderdate);
			...
			$current = $current->copy()->startOfMonth();

			$endDate = Carbon::parse($order->workorderduedate);
			...
			if($current->greaterThan($endDate))
			{
				$order->months = $months;
				continue;
			}
			...
			{
				if ($current->lessThan($currentMonth))
				{

					$month	= 	$current->format('m');
					$year  	= 	$current->format('Y');

					$record	= 	DB::table('mpr_tbl')
								->where('order_id', $order->orderid)
								->where('mpr_month', $month)
								->where('mpr_year', $year)
								->first();
			...
			if($mpr_month)
			{
				$months = $months->filter(function ($month) use ($mpr_month) {
					return $month['month'] == $mpr_month;
				});
			}

			if ($mpr_year)
			{
				$months = $months->filter(function ($month) use ($mpr_year) {
					return $month['year'] == $mpr_year;
				});
			}

			$order->months = $months->values();
		}

		$orders = $orders->filter(function ($order) {

			return $order->months->isNotEmpty();
		...
		return view(
			'admin/vendors/ajaxpages/pendingMprTable',
			[
				'data' => $orders
			]
		);
    ...
    public function vendorApprovedMpr(Request $request)
	{
        Session::put('adminmenu','mprmanagament');
		Session::put('adminsubmenu','approvedmprs');
		$userId		= 	$request->session()->get('userId');
		$vendorId	= 	$request->session()->get('vendorId');
		$subUserId	= 	$request->session()->get('subUserId') ?? 0;
		$issuper	= 	$request->session()->get('issuper');
		
		$records	=	DB::table('mpr_tbl as a')
							->select('c.departmentid','c.departmentname','c.isprojectmanager')
							->join('eoi_work_order as b','b.orderid','=','a.order_id')
							->join('department_tbl as c','c.departmentid','=','b.department_id')
							->where('a.vendor_id',$vendorId)
							->where('a.mpr_status','Verified')
							->where('a.is_invoiced',0)
							->when($subUserId > 0, function ($query) use ($subUserId) {
								$query->where('a.subUserId', $subUserId);
							})
		...
		return view('admin/vendors/vendor_approved_mprs',compact('records'));
    }

	public function vendorApprovedMprData(Request $request)
	{
		$vendorId     = $request->session()->get('vendorId') ?? 0;
		$subUserId    = $request->session()->get('subUserId') ?? 0;

		$mpr_month    = (int) $request->input('mpr_month', 0);
		$mpr_year     = (int) $request->input('mpr_year', 0);
		$departmentid = (int) $request->input('departmentid', 0);
		...
		$orders = DB::table('mpr_tbl as a')
			...
			->when($mpr_month > 0, function ($query) use ($mpr_month) {
				$query->where('a.mpr_month', $mpr_month);
			})
			->when($mpr_year > 0, function ($query) use ($mpr_year) {
				$query->where('a.mpr_year', $mpr_year);
			})
			->when($departmentid > 0, function ($query) use ($departmentid) {
			...
			->when($pagesearch !== '', function ($query) use ($pagesearch) {
				...
				$query->where(function ($q) use ($pagesearch) {
					...
					->orWhere('b.ordernumber','like','%' . $pagesearch . '%');
				});
			})
			->groupBy(
				'b.orderid',
		...
		$orderIds	= 	$orders->getCollection()->pluck('orderid')->values()->all();
		...
		foreach($orders as $order)
		{
			$order->mprs = $mprs->get($order->orderid, collect())->values();
		}
		return view('admin/vendors/ajaxpages/approvedMprTable',['data' => $orders]);
	}

    public function vendorInvoicedMpr(Request $request)
	{
        Session::put('adminmenu','mprmanagament');
		Session::put('adminsubmenu','invoicedmpr');
		$userId		= 	$request->session()->get('userId');
		$vendorId	= 	$request->session()->get('vendorId');
		$subUserId	= 	$request->session()->get('subUserId') ?? 0;
		$issuper	= 	$request->session()->get('issuper');
		...
		return view('admin/vendors/vendor_invoiced_mprs',compact('records'));
    }


	public function vendorInvoicedData(Request $request)
	{
		$vendorId		=	$request->session()->get('vendorId') ?? 0;
		$subUserId 		= 	$request->session()->get('subUserId') ?? 0;
		...
		$invoices	=	DB::table('invoice_mpr as a')
						->select(
							'a.voucher_id',
							'a.invoice_number',
							'a.invoice_date',
						...
						->when($pagesearch !== '', function ($query) use ($pagesearch) {
							$query->where('a.invoice_number','like','%' . $pagesearch . '%')
								  ->orwhere('b.ordernumber','like','%' . $pagesearch . '%');
						})
		...
		{
			$mprIds = array_filter(explode(',', $invoice->mpr_ids));
			if(!empty($mprIds))
			{
				$invoice->mprs	=	DB::table('mpr_tbl as a')
									->select('a.*')
									->whereIn('a.mpr_id', $mprIds)
									->where('a.vendor_id', $invoice->vendor_id)
									->where('a.is_verified', 1)
									->where('a.is_invoiced', 1)
		...
		*/
			...
			if(!empty($mprIds))
			{
				$invoice->mprs = DB::table('mpr_tbl as a')
					...
					->map(function ($mpr) {
						...
						$mpr->attendance_summary = DB::table('mpr_attendance_summary as s')
							->leftJoin(
								'eoi_resource_deployment as r',
								'r.deploymentid',
								'=',
								's.deployment_id'
							)
							->select(
								...
								's.actual_salary',
								's.calculated_salary',
								's.calculated_operating',
								's.calculated_gst',
								's.net_salary',
								's.deduction',
								's.approved_salary',
								'r.name as resource_name',
								'r.mobilenumber as resource_mobilenumber',
								'r.email as resource_email'
							)
							->where('s.mpr_id', $mpr->mpr_id)
							->get();
		...
		return view('admin/vendors/ajaxpages/invoicedDataTable',['data' => $invoices]);
	}
}
